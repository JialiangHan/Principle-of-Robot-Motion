...
class Map:
    ...
    def generate_boundary(self):
        ...
        edge_list = []
        for i in range(2):
            node_list.append(Node(self.size[i], self.size[2]))
            node_list.append(Node(self.size[i], self.size[3]))
        ...
        self.boundary = Polygon(edge_list)
    ...
    def generate_obstacle(self):
        ...
        for i in range(self.number_of_obstacle):
            ...
            for j in range(len(convexhull.hull) - 1):
                edge_list.append(Edge(convexhull.hull[j], convexhull.hull[j + 1]))
